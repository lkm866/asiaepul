...
class DataProcessor:
    ...
    async def read_and_merge_data(self):
        if self.total_data is None:
            # 데이터 읽기 및 병합
            data_frames = []
            # 현재 스크립트의 디렉토리를 기준으로 데이터 폴더의 경로를 설정
...
class CategoriesCache:
    ...
    async def _generate_products(cls, query: str = "") -> List[Category]:
        ...
        # CSV 파일에서 제품 정보 가져오기
        base_directory = os.path.abspath(os.path.dirname(__file__))
        category_file_path = os.path.join(base_directory, "data", "category.csv")

        with open(category_file_path, newline='') as csvfile:
...
class AIData(BaseModel):
    ...
    async def run_LTSF_NLinear(code: int):
        ...
        predict_dir = os.path.join(base_directory, "data", "predict")
        os.makedirs(predict_dir, exist_ok=True)

        file_path = os.path.join(predict_dir, f"category_{code}.csv")
        data_final.to_csv(file_path)
        
        raw = pd.read_csv(file_path, index_col=['date_day'], parse_dates=True)
    ...
    async def run_LSTM(code: int):
        ...
        # predict_dir 경로를 초기화합니다.
        predict_dir = os.path.join(base_directory, "data", "predict")
        os.makedirs(predict_dir, exist_ok=True)  # 해당 경로에 디렉토리가 없으면 생성합니다.
