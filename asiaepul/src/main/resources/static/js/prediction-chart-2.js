document.addEventListener("DOMContentLoaded", async () => {
    const dom = document.getElementById('prediction-chart-2');
    ...
    async function drawChart(modelName) {
		...
		const url = `http://localhost:8000/prediction-2?model_name=${modelName}`;
...
async function updateChartData(modelName) {
    const url = `http://localhost:8000/prediction-2?model_name=${modelName}`; // 모델 이름에 따른 URL
